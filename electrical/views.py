...
def view_sales(request):
    ...
    if request.method == "POST":
        ...
        # Correct the query using the __range lookup for between dates
        book = Booking.objects.filter(book_date__range=[fromDate, toDate])

    d = {'book': book}
    return render(request, 'view_sales.html', d)
...
def Add_Cart(request,pid):
    ...
    if request.method=="POST":
        ...
        cqnt = request.POST['cqnt']
        Cart.objects.create(profile=profile, product=product, cqnt=cqnt)
...
def view_cart(request):
    ...
    if cart:
        for i in cart:
            cqnt = i.cqnt
            total += i.product.price * i.cqnt
            num1 += 1
            book_id = book_id + "." + str(i.product.id) + "." + str(cqnt)
        ...
        d = {
            ...
            'num1': num1,
            'book': book_id,
            'cqnt': 0,  # Set cqnt to 0 if cart is empty
            'message': message1,
        }
...
def booking_detail(request,pid,bid):
    ...
    for j in li:
        if user1!= j :
            li2.append(int(j))
        cqnt = li2[1] if len(li2) >= 3 else 0
    ...
    d = {'profile':profile,'cart':cart,'total':total,'num1':num1,'book':li2,'product':product,'total':book,'cqnt':cqnt}
...
def admin_booking_detail(request,pid,bid,uid):
    ...
    for i in cart:
        total+=i.product.price
        num1+=1
    d = {'profile':profile,'cart':cart,'total':total,'num1':num1,'book':li2,'product':product,'total':book,'third_element':third_element}
